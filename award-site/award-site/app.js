...
function bindEvents() {
  ...
  els.printPreview.addEventListener("click", () => {
    ...
    waitForPrintAssets(els.printRoot).then(() => {
      window.print();
    });
  });
...
function waitForFonts() {
  ...
  return Promise.all([
    document.fonts.load('22px "EduKai"'),
    document.fonts.load('22px "標楷體"'),
    document.fonts.ready,
  ]).then(() => undefined);
}

function waitForPrintAssets(root) {
  return Promise.all([waitForImages(root), waitForFonts()]).then(() => undefined);
}
